...
**Wallet**:
A container of money owned by a User, such as a bank card, a cash stash, or a savings pot. Every Wallet holds exactly one currency. A User's Wallets form an ordered list.
...
**Default Currency**:
The single currency a User chooses to view all their money in. Every Transaction is converted into it for analysis. Changeable in settings.
...
**Tag**:
An optional free-form marker attached to a Transaction for cross-cutting grouping (e.g. "vacation-2026"). A Transaction may carry many Tags.
_Avoid_: label, hashtag

**Unassigned**:
The state of a Transaction that has no Wallet because, at capture, no Wallet matched its currency. Kept in its original currency, included in analysis via the Exchange Rate, and shown as needing a Wallet. Never rejected at capture.
_Avoid_: orphan, pending, inbox

**Shortcut**:
An Apple Shortcuts automation on the User's iPhone, generated by June from the User's Categories, that captures a Transaction by sending amount, currency, category slug, and description to June. It is the primary capture method; the in-app form is secondary. A Shortcut never names a Wallet; the Wallet is resolved by Wallet Order.
...
**OAuth account**:
An identity at an external provider (e.g. Google) linked to a User for sign-in. One User may have several. A User can also sign in by email magic link, which is not an OAuth account; the User's email is the identity. June never stores passwords.
