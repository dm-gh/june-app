...
- Recurring or scheduled Transactions (added in Round 3)
...
## Round 2 (designed 2026-09-12, implemented 2026-09-13; see docs/figma.md "Round 2 screens")
...
## Round 3 (designed and implemented 2026-09-13; see docs/figma.md "Round 3 screens")
- Recurrings: a named template for a Change (amount, currency, Wallet, Category, description, Tags) with a Schedule and an Auto flag, on a new More tab. The Schedule is stored as a standard five-field cron expression built by a small generator the User never sees the output of (daily, weekly on one or more weekdays, monthly on one or more days, yearly on a day of a month) or as a single once date with no expression; the next due date is stored alongside and advanced on every firing. Auto firing runs in the api process itself, an hourly tick applying every Recurring whose due date has passed in UTC and dating each Change on its due date, catching up missed dates after downtime; turning Auto on or editing the Schedule recomputes the next date from today. With Auto off the Schedule is optional; the User fires from the Recurring's page with Submit (fire on the due date as is) or Edit & submit (the Change form prefilled). A manual Recurring's card carries Submit; an auto one only offers it on its page. Overdue is shown quietly. A Recurring whose Wallet was deleted fires Unassigned and a "Some items need attention" banner appears on every screen until it is fixed; a deleted Category means Uncategorised. Only Changes for now; recurring Exchanges (auto-balancing between Wallets) are a later round. No link between a Recurring and what it produced (ADR-0005). The earlier "out of the MVP" entry for recurring Transactions is superseded by this round.
- Loans: money lent (positive) or borrowed (negative) outside June's Wallets, as a list on the More tab: amount, currency, description. Settle opens the Change form prefilled with the remaining amount, sign toward zero, Hidden from analysis on, Wallets in the Loan's currency; one server call writes the Change and moves the Loan's amount by the opposite of the Change, so settling past zero flips Lent into Borrowed and a negative settlement lends more. The amount is stored, not derived, and never linked to the Change (ADR-0005). An Archived flag is set when Settling reaches zero and can be set or cleared by hand; archived Loans live behind an "Archive" link like the analysis All pages; delete works in both states. Loans appear in no analysis and no Balance.
- Shell: four tabs, Analysis | Transactions | More | Settings, icons only on the phone (the active tab shows its label under the icon and keeps its height when the plus slides up over Transactions), icon plus label on the desktop rail. More has no period and no filter: two sections, Recurring then Loans, each with a "+ Add" in its heading like Settings.
