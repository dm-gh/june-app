...
- Transactions are editable and deletable in the app, including their date. A single Transaction opens in a read-only view; Edit and Delete sit behind an options menu. Long-pressing a card enters selection mode for bulk actions: bulk Edit changes only Wallet, Category and Tags for every selected item, bulk Delete removes them. Every delete asks for confirmation first.
