...
- **Filters** (Transactions · Filtered, Transactions · Filters, and the header of Analysis · Extended). One filter set shared by Transactions and Analysis, like the period, kept in the URL (`?type=expense&cat=<slug,…>&tag=<…>&wallet=<id,…>`) so a link carries it. Under the period row sits a ghost Filter button with the Phosphor Funnel glyph followed by the active filters as SM chips, each ending in × to remove it: type in coral, categories in their own colour, tags with the # prefix in yellow, wallets in sky. The button opens the Filter sheet from the bottom over the scrim, built like the Period sheet: Type as a three-way segmented control (All / Expense / Income), then Categories, Tags and Wallets as wrapping chip groups where a selected chip keeps its colour and gains the SM hard shadow while an unselected one is paper; Clear (ghost) and Apply (primary) at the foot. Init and Exchange rows are hidden whenever a type filter is on. With no filter active (Transactions · No filter, Transactions · Filters · Empty) the row holds only the Filter button, so the header keeps its height; in the sheet All is the active type, every chip is paper, Clear is disabled and Apply stays enabled.
...
- **Analysis · Extended**. Sections in order: Spent / Income cards; Per day (one bar per day, today in accent, later days faded, a coral running-total line, the daily average in the heading); By category; Income vs expense (one column per day: income up in green, expense down in coral from a zero line, net in the heading); By tag; By wallet. Every chart shows days while the period is a month or shorter and months beyond that. Every section obeys the filters. The old Over time chart and the Wallet balances list are gone: balances live in Settings.
- **Analysis · Categories / Tags / Wallets** (reached by the All › button of the matching section, which shows at most five rows). Header: Back, the section title, the period row and the filter row. The categories page opens with the Share of spending donut and legend; tags and wallets have no colours, so they open straight into the list. Every row carries the amount, its share as a percentage, and a checkbox that toggles that item in the shared filter, so a breakdown page doubles as a filter picker.
