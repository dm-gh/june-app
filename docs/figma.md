...
- **Shortcut** mirrors the guide already in code (`/settings/shortcut`): numbered steps with accent number tiles, step 1 the capture URL card, then each Shortcuts action as a Shortcut action instance (tinted tile, name, up to three label/value rows) with the user's slugs, currencies and URL, the JSON field table in step 8, and a Show Notification finish. The Shortcut action component exposes Name, three Label/Value pairs and Show row 2/3 booleans; the tile fill is overridden per action kind.
